...
// Naive recursive way.
int knapsackNaiveRec(int n, int *weight, int *profit, int i, int total,
    int weight_max)
{
  if (weight_max < 0) {
    return 0;
  ...
  int p1 = knapsackNaiveRec(n, weight, profit, i + 1,
      total + profit[i], weight_max - weight[i]);
  int p2 = knapsackNaiveRec(n, weight, profit, i + 1,
      total, weight_max);
  return max(p1, p2);
}
