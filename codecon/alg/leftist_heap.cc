...
public:
  ...
  const Value& top() const { return root_->v; }
...
private:
  ...
  static Node *merge_node(Node *a, Node *b) {
    ...
    if (less(a->v, b->v)) {
      ...
      // If both keys are equal, keep merging. We'll dedup in `pop`. But if
      // we really do this, `size` become meaningless. We can pop then less
      // size! So, let's allow duplicates. Additional `pop_unique`.
