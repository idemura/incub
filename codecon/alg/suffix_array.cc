...
vector<int> suffix_array(const string &s) {
  ...
  for (int i = 0; i < n; i++) sp[i].set(i, s[i], 0);
  pairs_to_ord(sp, ord);
...
int suffix_array_lcp(const vector<vector<int>> &ord, int i, int j) {
  // Tricky moment: because a and _ (symbol after string's end) and both zeros,
  // we may mistakingly get LCP of 'b_' and 'ba' is 2 instead of 1. To avoid
  // this, clamp to the max string size.
