...
Subsums rec_step(const vector<int> &a, i64 mod, int l, int r, i64 &max_mod) {
  ...
  if (r - l == 1) {
    ...
    // Left and right already checked. Make ranges of one element on left and
    // maximum possible on the right.
    for (auto x : st_l.subsums) {
      // We store sum from the beginning. So, the sum to the end is total minus
      // sum to the left.
      auto r_sum = (st_l.total + mod - x) % mod;
      // Apparently, max possible sum is m-1. Try to find this in the right
      // part.
