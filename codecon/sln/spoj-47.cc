...
struct Problem {
  ...
  Problem(): f(), e(), a(), b(), first(), step(), am(), am_n(),
      ar(), br(), mark(), mval() {}
  ...
  bool canMove()
  {
    ...
    // Make adjacency matrix for elevators. Two elevators are connected, if they
    // have some floor where both stop.
    for (int i = 0; i < e; i++) {
      for (int j = i + 1; j < e; j++) {
  ...
  {
    // Fi + Si * Ki = Fj + Sj * Kj => Fi - Fj = Sj * Kj - Si * Ki. So, minimum
    // integer can be represented this way is `g = gcd(Si, Sj)`. Therefore, if
    // `Fi - Fj` divides by `g`, elevators are connected. Have to check if
    // connection is in the limits.
    int g, ki, kj;
