...
public:
  i64 countVotes(const vector<string> &votes, string website) {
    ...
    // Do Floyd-Warshall to decide about cycles (connection, not path length).
    m_closure_ = m_;
    for (int k = 0; k < n; k++) {
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
  ...
  vector<int> parse(const string &s) {
    ...
    for (int i = 0, j = 0; j != string::npos; i = j + 1) {
      ...
      if (it == ix_map_.end()) {
        // If I do on the next like, TopCoder assigns 1 to the first string.
