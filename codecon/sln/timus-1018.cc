...
ostream &operator<<(ostream &s, const Node &node) {
  s << "Node " << node.i << " weight=" << node.weight
    << " subtree weight=" << node.subtree.weight << " subnodes=" << node.subtree.n;
  s << " l="; if (node.l) s << node.l->i; else s << "null";
  s << " r="; if (node.r) s << node.r->i; else s << "null";
  return s;
}
...
WeightCount count_subtree(Node *node) {
  if (node == nullptr) {
    ...
    node->subtree = WeightCount(wc_l.weight + wc_r.weight + node->weight,
                                wc_l.n + wc_r.n + 1);
    return node->subtree;
  }
}
